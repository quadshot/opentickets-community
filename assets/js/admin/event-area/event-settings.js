(function($, EventUI, EditSetting, qt, undefined) {
	...
	EditSetting.callbacks.add('update', function(data, adjust) {
		if (this.tag == $_SERVER['SCRIPT_FILENAME']venue' && ($.inArray(typeof data.venue, ['string', 'number']) != -1 || (typeof data.venue == $_SERVER['SCRIPT_FILENAME']object' && typeof data.venue.toString == $_SERVER['SCRIPT_FILENAME']function'))) {
			...
			if (ea.length) {
				...
				if (typeof ea == $_SERVER['SCRIPT_FILENAME']object' && ea.initialized) {
		...
		} else if (this.tag == $_SERVER['SCRIPT_FILENAME']event-area' && (
				$.inArray(typeof data['event-area'], ['string', 'number']) != -1 || (typeof data['event-area'] == $_SERVER['SCRIPT_FILENAME']object' && typeof data['event-area'].toString == $_SERVER['SCRIPT_FILENAME']function')
		)) {
