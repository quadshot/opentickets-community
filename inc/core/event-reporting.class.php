...
class qsot_reporting {
	...
	public static function add_view_links($report_type, $req, $csv) {
		...
		if (is_array($csv) && isset($csv['url']) && !empty($csv['url']))
			$links['csv'] = sprintf('<a href="%s" title="%s">%s</a>', esc_attr($csv['url']), __('Download CSV','opentickets-community'), __('Download CSV','opentickets-community'));
		...
		if (isset($req)) {
			...
			$links['pf'] = sprintf('<a href="%s" title="%s" target="_blank">%s</a>', esc_attr($link), __('Printer Friendly Version','opentickets-community'), __('Printer Friendly Version','opentickets-community'));
		}
		?>
		<?php if (count($links)): ?>
...
abstract class qsot_admin_report {
	protected static $report_name = __('Report','opentickets-community');
	...
	public static function printer_friendly_header($args='') {
		...
		if ( is_network_admin() )
			$admin_title = __('Network Admin','opentickets-community');
		elseif ( is_user_admin() )
			$admin_title = __('Global Dashboard','opentickets-community');
		else
			$admin_title = get_bloginfo( 'name' );

		if ( $admin_title == $title )
			$admin_title = sprintf( __('%1$s &#8212; WordPress','opentickets-community'), $title );
		else
			$admin_title = sprintf( __('%1$s &lsaquo; %2$s &#8212; WordPress','opentickets-community'), $title, $admin_title );
	...
	public static function csv_path_notice() {
		if (self::$bad_path) {
			...
			printf(__("Could not create the report cache directory. Make sure that the permissions for '%s' allow the webserver to create a directory, and try again.",'opentickets-community'), $path);
			echo '</div>';
		}
	}
	...
	protected function _report_error($msg='') {
		$msg = $msg ? $msg : __('An un expected error occurred during report generation.','opentickets-community');
	...
	protected function _draw_results($template, $tallies, $csvs, $events, $errors) {
		if (!empty($template)) {
			...
			echo '<p>'.__('Could not find the report result template. Below is the list of raw CSV results.','opentickets-community').'</p><ul>';
			foreach ($csvs as $event_id => $csv) {
				echo sprintf(
					'<li><a href="%s" title="%s">%s</a></li>',
					esc_attr($csv['url']),
					esc_attr(__('View report for','opentickets-community').' '.$events[$event_id.'']['title']),
					__('Report for','opentickets-community').' '.$events[$event_id.'']['title']
				);
			}
			echo '</ul>';
		}
	...
	protected function _payment_method($order, $fallback='') {
		return isset($order['_payment_method']) && !empty($order['_payment_method'])
			...
			: ( isset($order['_order_total']) && $order['_order_total'] > 0 ? __('(unknown)','opentickets-community') : __('-free-','opentickets-community') );
	}
